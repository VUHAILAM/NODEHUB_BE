...
func (g *CandidateGorm) GetAllCandidateForAdmin(ctx context.Context, name string, page int64, size int64) (*models2.ResponsetListCandidateAdmin, error) {
	...
	var total int64
	//search query
	data, err := db.Raw(`select  c.candidate_id, a.email, CONCAT_WS(" ", c.last_name, c.first_name) AS 'fullname', c.first_name, c.last_name, c.gender, c.birth_day, c.address, c.avatar, c.banner, c.phone, c.find_job, 
